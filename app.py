...
def render_accum_plan():
    ...
    if combos:
        ...
        if comp:
            with st.expander(f"📊 청산 규칙 × 비중 {len(comp)}개 조합 비교", expanded=True):
                st.dataframe(pd.DataFrame(comp), use_container_width=True, hide_index=True)
                st.caption(
                    "· **'200일선 복귀'는 승률이 구조적으로 높게 나옵니다.** 200일선 아래에서 사서 "
                    "위에서 팔기 때문입니다. 대신 수익이 작고 투입률이 낮습니다.  \n"
                    "· **보유 기간 규칙(1·3·6·12개월)은 승률이 낮지만 수익 분포가 넓고 투입률이 높습니다.** "
                    "종목 간 차이를 보려면 이쪽 변별력이 가장 큽니다. 기간을 늘릴수록 "
                    "투입률과 수익 폭이 함께 커지고 버텨야 할 미실현 손실도 깊어집니다.  \n"
                    "· 1개월은 아직 회복 못 한 상태로 끝나는 경우가 많아 승률이 가장 낮게 나옵니다. "
                    "대신 표본이 가장 많아 **'반등이 빠른 종목인지'** 를 보는 데 유용합니다.  \n"
                    "· 같은 종목을 1개월부터 12개월까지 나란히 보면 "
                    "**'더 오래 들고 있는 게 유리한 종목인지'** 가 보입니다.")
...
- **'200일선 복귀' 승률은 구조적으로 높습니다.** 200일선 아래에서 사서 위에서 팔기
  ...
  종목 간 실력 차이를 보려면 **보유 기간 규칙(1·3·6·12개월)** 쪽 숫자를 보세요.
- **보유 기간 규칙은 진입 후 해당 기간이 지나면 무조건 청산합니다.** 손절도 익절도 없이
  기간만 채우므로 승률이 가장 정직하게 나옵니다. 기간이 짧으면 표본이 많아지지만
  아직 회복 못 한 상태로 끝나는 경우가 늘고, 길면 반대입니다.
